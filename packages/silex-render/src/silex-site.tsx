...
import { downloadPublishedFile, parseSilexPublishedRef } from "./published";
import { renderSilexHtmlWithEmbeds } from "./embeds";

/**
 * Renders an org's Silex/Nextcloud-published site: download the static HTML,
 * sanitize it, rewrite asset URLs to the host app's asset proxy, bind live DB
 * values into data-trait slots, and swap <eac-embed> placeholders for live
 * React. Shared by arts-collective and the dedicated per-org apps.
 *
 * `cssLinks` are extra stylesheet hrefs to inject (e.g. the connector template
 * CSS routes). Each host app supplies the ones it serves; the published HTML
...
/**
 * Derive the published ref for a specific page by swapping the entry filename
 * (e.g. `.../published/index.html` + page "centers" → `.../published/centers.html`).
 */
function pageRef(publishedPath: string | null, page?: string): string | null {
  if (!publishedPath) return null;
  if (!page) return publishedPath;
  return publishedPath.replace(/[^/]+\.html$/i, `${page}.html`);
}

export async function SilexSite({
  org,
  cssLinks = [],
  page,
}: {
...
/**
 * Convenience wrapper for per-org apps whose root *is* the site: loads the org
 * by slug, then renders. Returns null if the org doesn't exist.
 */
export async function SilexSiteBySlug({
  slug,
  cssLinks,
  page,
}: {
