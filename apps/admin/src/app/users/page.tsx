...
export default function UsersPage() {
  ...
  const [ncUsers, setNcUsers] = useState<NextcloudUser[]>([]);
  ...
  const loadNcUsers = async () => {
    setLoadingNc(true);
    try {
      const response = await fetch('/api/nextcloud/nc-users', { credentials: 'include' });
      if (response.ok) {
        const data = await response.json();
        setNcUsers(data.users || []);
      }
    } catch (error) {
      console.error('Error loading NC users:', error);
    } finally {
      setLoadingNc(false);
    }
  };
  ...
  const syncSelectedUsers = async () => {
    for (const userId of selectedForSync) {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          console.error(`Sync failed for ${userId}:`, err.error);
        }
      } catch (error) {
        console.error(`Sync error for ${userId}:`, error);
      } finally {
  ...
  const toggleSyncSelect = (userId: string) => {
  ...
  return (
    <Container size="xl" py="xl">
      <Stack gap="xl">
          ...
          <Group>
            ...
            {selectedForSync.size > 0 && (
              <Button
                ...
                loading={syncingUsers.size > 0}
              >
                Sync {selectedForSync.size} to Nextcloud
              </Button>
            )}
          </Group>
        </Paper>
                  ...
                  <Table.Tr key={user.id}>
                    <Table.Td>
                      {!user.nextcloud_synced && (
                        <Checkbox
                          checked={selectedForSync.has(user.id)}
                          onChange={() => toggleSyncSelect(user.id)}
                          disabled={syncingUsers.has(user.id)}
                          size="xs"
                        />
                      )}
                      {syncingUsers.has(user.id) && <Loader size="xs" />}
                    </Table.Td>
                    <Table.Td>
                      <Group gap="sm">
        ...
        {/* Nextcloud Users Table */}
        <Paper withBorder>
          <Group justify="space-between" p="md" pb={0}>
            <Group gap="sm">
              <Cloud size={20} />
              <Title order={4}>Nextcloud Users</Title>
              <Badge variant="light" size="lg">
                {ncUsers.length}
              </Badge>
            </Group>
          ...
          {loadingNc ? (
            ...
            <Center h={100} pb="md">
              <Text c="dimmed" size="sm">No Nextcloud users found</Text>
            </Center>
          ) : (
            <Table striped highlightOnHover>
              <Table.Thead>
              ...
              <Table.Tbody>
                {ncUsers.map((ncUser) => (
                  <Table.Tr key={ncUser.id}>
                    <Table.Td>
                      <Text size="sm" fw={500} c="dark" style={{ fontFamily: 'monospace' }}>
                        {ncUser.id}
                      </Text>
                    </Table.Td>
                    <Table.Td>
                      <Text size="sm" c="dark">{ncUser.displayName || '—'}</Text>
                    </Table.Td>
                    <Table.Td>
                      <Text size="sm" c="gray.7" style={{ fontFamily: 'monospace' }}>
                        {ncUser.email || '—'}
                    ...
                    <Table.Td>
                      <Group gap={4}>
                        {(ncUser.groups || []).slice(0, 2).map((g) => (
                          <Badge key={g} size="xs" variant="outline" color="gray">
                            {g}
                          </Badge>
                        ))}
                        {(ncUser.groups || []).length > 2 && (
                          <Text size="xs" c="dimmed">
                            +{ncUser.groups.length - 2}
                          </Text>
                        )}
                    ...
                    <Table.Td>
                      <Text size="sm" c="dimmed">
                        {ncUser.lastLogin
                          ? new Date(ncUser.lastLogin * 1000).toLocaleDateString()
                          : 'Never'}
                      </Text>
                    </Table.Td>
                  </Table.Tr>
