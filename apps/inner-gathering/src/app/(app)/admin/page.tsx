...
const ORG_MEDIA_ROOT = 'EAC_Network/inner_group';
const ORG_IMAGES_ROOT = `${ORG_MEDIA_ROOT}/Media/Images`;
...
export default function AdminPage() {
  ...
  const [mediaPath, setMediaPath] = useState(ORG_IMAGES_ROOT);
  ...
  const selectedGalleryImages = Array.isArray((imageSpaces as any).gallery?.images)
    ? ((imageSpaces as any).gallery.images as string[])
    : [];
  ...
  const assignImageToSpace = (path: string) => {
    ...
    let nextImageSpaces: Record<string, any> = {};
    setConfig((prev) => {
      nextImageSpaces = {
      ...
      return {
        ...prev,
        image_spaces: nextImageSpaces,
      };
    });
    void patch('image_spaces', nextImageSpaces as SiteConfig);
  };
  ...
  const toggleGalleryImage = (path: string) => {
    ...
    let nextImageSpaces: Record<string, any> = {};
    setConfig((prev) => {
      const curr = prev.image_spaces as Record<string, any>;
      const currentImages = Array.isArray(curr.gallery?.images) ? curr.gallery.images as string[] : [];
      const nextImages = currentImages.includes(normalizedPath)
        ? currentImages.filter((img) => img !== normalizedPath)
        : [...currentImages, normalizedPath];
      nextImageSpaces = {
        ...curr,
        gallery: {
          ...(curr.gallery ?? {}),
    ...
    <div style={styles.page}>
      ...
      <section style={styles.card}>
        ...
        <div style={styles.folderBar}>
          ...
          <button type="button" onClick={() => loadMediaItems(ORG_MEDIA_ROOT)} style={styles.smallBtn}>Go to groups root</button>
        ...
        <p style={{ ...styles.muted, marginBottom: '0.35rem' }}>Gallery images selected: {selectedGalleryImages.length}</p>
          ...
          <>
            <div style={styles.galleryFolderGrid}>
              {folderItems.map((folder) => (
                <button
                  ...
                  onClick={() => loadMediaItems(normalizeMediaPath(folder.filename))}
                >
                  {folder.basename}
                </button>
              ))}
            ...
            <div style={styles.galleryImageGrid}>
              {imageItems.length === 0 ? (
                ...
                const normalizedPath = normalizeMediaPath(file.filename);
                const assigned = selectedSpace.path === normalizedPath;
                const inGallery = selectedGalleryImages.includes(normalizedPath);
                return (
                  <div key={file.filename} style={assigned ? styles.galleryCardSelected : styles.galleryCard}>
                    ...
                    <button type="button" style={styles.smallBtnMuted} onClick={() => toggleGalleryImage(normalizedPath)}>
