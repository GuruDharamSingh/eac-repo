...
function ReplyBody({ content }: { content: string }) {
  ...
  const needsClamp = content.length > BODY_CLAMP_CHARS;
  const shown =
    expanded || !needsClamp ? content : content.slice(0, BODY_CLAMP_CHARS) + "…";

  return (
    <>
      <p className={styles.replyBody}>{shown}</p>
      {needsClamp && (
        <button
          type="button"
...
export function RepliesThread({ replies, threadSlug, isAuthed }: RepliesThreadProps) {
