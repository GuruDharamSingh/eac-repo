...
export async function createMeeting(params: {
  ...
  // Calculate duration in minutes if endTime is provided
  let durationMinutes: number | null = null;
  if (params.endTime) {
    durationMinutes = Math.round((params.endTime.getTime() - params.startTime.getTime()) / (1000 * 60));
  }

  const meetingId = generateId();

  const scheduledAt = params.startTime;
  ...
  const [meeting] = await db`
    INSERT INTO meetings (
      ...
      ${scheduledAt}, ${durationMinutes}, ${params.location || null},
